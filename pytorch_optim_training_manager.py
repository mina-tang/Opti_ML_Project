...
class train_manager(object):
    ...
    def train(self, epochs, eval_all_epochs=False, eval_mode=None, verbose=True, is_LSTM=False):
        train_losses = []
        eval_losses = []
        for epoch in range(epochs):
            ...
            if eval_all_epochs:
                eval = self.eval_model(eval_mode=eval_mode, is_LSTM=is_LSTM)
    ...
    def eval_model(self, eval_mode=None, is_LSTM=False):
        self.model.eval()
        total_loss = 0
        total_correct = 0.
        total_samples = 0.
